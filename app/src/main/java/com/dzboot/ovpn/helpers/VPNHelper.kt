...
object VPNHelper {
    ...
    inline fun <reified T : OpenVPNService> startVPNIntent(context: Context, selectedServer: Server? = null) {
        ...
        with(Intent(context, T::class.java)) {
            ...
            if (selectedServer == null) {
                putExtra(OpenVPNService.IS_AUTO_CONNECT, true)
            } else {
                putExtra(OpenVPNService.SELECTED_SERVER_ID_EXTRA, selectedServer.id)
                putExtra(OpenVPNService.IS_AUTO_CONNECT, false)
            }
